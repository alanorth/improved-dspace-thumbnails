...
QUALITY_START=$(jq -r '.recipes.webp.quality_start' < src/recipes/im7.json)
QUALITY_END=$(jq -r '.recipes.webp.quality_end' < src/recipes/im7.json)
QUALITY_STEP=$(jq -r '.recipes.webp.quality_step' < src/recipes/im7.json)
ENCODE_EXTENSION=$(jq -r '.recipes.webp.encode_extension' < src/recipes/im7.json)
ENCODE_CMD=$(jq -r '.recipes.webp.encode_cmd' < src/recipes/im7.json)
EXPORT_TO_PNG=$(jq -r '.recipes.webp.export_to_png' < src/recipes/im7.json)
RESULTS_FILE=results/im7/webp.csv
