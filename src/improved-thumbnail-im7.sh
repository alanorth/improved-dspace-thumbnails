# Create a thumbnail using ImageMagick 7. For now this uses a Docker image be-
# cause I *think* Ghostscript is broken in Arch Linux (CMYK PDFs rendering in-
# correctly).

QUALITY_START=$(jq -r '.recipes.jpeg.quality_start' < src/recipes/im7.json)
QUALITY_END=$(jq -r '.recipes.jpeg.quality_end' < src/recipes/im7.json)
QUALITY_STEP=$(jq -r '.recipes.jpeg.quality_step' < src/recipes/im7.json)
ENCODE_EXTENSION=$(jq -r '.recipes.jpeg.encode_extension' < src/recipes/im7.json)
ENCODE_CMD=$(jq -r '.recipes.jpeg.encode_cmd' < src/recipes/im7.json)
EXPORT_TO_PNG=$(jq -r '.recipes.jpeg.export_to_png' < src/recipes/im7.json)
RESULTS_FILE=results/im7/jpeg.csv
